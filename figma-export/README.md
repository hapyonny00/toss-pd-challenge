...
각 파일은 **`shell.html` 네비게이션 레일(좌측 다크 플로팅 슬래브) + 본문 화면**을 iframe 없이 flat DOM으로 합친 전체 화면입니다. 외부 폰트(Pretendard CDN)와 프로필 이미지(pravatar) URL만 참조하고, 그 외 스타일·마크업은 전부 파일 안에 들어 있습니다. 인터랙션 스크립트는 제거돼 있어 **정적 스냅샷**이에요.
